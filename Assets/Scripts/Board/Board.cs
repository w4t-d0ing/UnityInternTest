...
public class Board
{
    ...
    public List<Cell> GetAllCellsWithItem()
    {
        List<Cell> cells = new List<Cell>();
        for (int x = 0; x < boardSizeX; x++)
            for (int y = 0; y < boardSizeY; y++)
                if (!m_cells[x, y].IsEmpty) cells.Add(m_cells[x, y]);
        return cells;
    }
    ...
    internal void Fill()
    {
        ...
        int setsOf3 = totalCells / 3;
        var allTypes = (NormalItem.eNormalType[])Enum.GetValues(typeof(NormalItem.eNormalType));
        int numTypes = allTypes.Length;

        List<NormalItem.eNormalType> itemPool = new List<NormalItem.eNormalType>(setsOf3 * 3);
        for (int i = 0; i < setsOf3; i++)
        {
            NormalItem.eNormalType type = allTypes[i % numTypes];
            itemPool.Add(type);
            itemPool.Add(type);
            itemPool.Add(type);
        }
        ...
        int poolIndex = 0;
        for (int x = 0; x < boardSizeX && poolIndex < itemPool.Count; x++)
        {
            for (int y = 0; y < boardSizeY && poolIndex < itemPool.Count; y++)
            {
